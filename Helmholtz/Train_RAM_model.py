...
def train_RAM_model(a1,a2,k):
    ...
    class Model:
        ...
        def true_loss(self, loss_e, loss_l):
            return loss_e#torch.exp(-self.x_f_s.detach()) * loss_e + torch.exp(-self.x_label_s.detach()) * loss_l
        ...
        def run_AM_AW1(self):
            ...
            error = 1
            # for move_count in range(AM_count):
            while error > 3e-3 and move_count < AM_count:
                self.optimizer_LBGFS = torch.optim.LBFGS(self.net.parameters(), lr=lbgfs_lr,
                                                        max_iter=lbgfs_iter)
                optimizer_adam = torch.optim.Adam(self.net.parameters(), lr=adam_lr)
    ...
    def x_f_loss_fun(xy, train_U):
        ...
        u_x1x1 = torch.autograd.grad(u_x1, xy, grad_outputs=torch.ones_like(u_x1), create_graph=True)[0][:, 0].unsqueeze(-1)
        u_x2x2 = torch.autograd.grad(u_x2, xy, grad_outputs=torch.ones_like(u_x2), create_graph=True)[0][:, 1].unsqueeze(-1)
        # u_exact = torch.sin(a1 * torch.pi * xy[:, [0]]) * torch.sin(a2 * torch.pi * xy[:, [1]])
        # f = -(a1 * torch.pi) ** 2 * u_exact - (a2 * torch.pi) ** 2 * u_exact + k ** 2 * u_exact - \
        #     (u_x1x1 + u_x2x2 + k ** 2 * u)
        x = xy[:, [0]]
        y = xy[:, [1]]
        q =( (2*(x**2 + y**2 - 2) - (a1**2 + a2**2)*torch.pi**2*(x**2-1)*(y**2-1) + k**2*(x**2-1)*(y**2-1)) * sin(a1*torch.pi*x)*sin(a2*torch.pi*y)
        + 4*torch.pi*(a1*x*(y**2-1)*cos(a1*torch.pi*x)*sin(a2*torch.pi*y) + a2*y*(x**2-1)*sin(a1*torch.pi*x)*cos(a2*torch.pi*y)) ).detach()
        f =q-(u_x1x1 + u_x2x2 + k ** 2 * u)
        return f
    ...
    def draw_residual(a1,a2,k):
        ...
        plt.tight_layout()
        plt.savefig(f'Helmholtz_residual-RAM-AW_a1={a1}_a2={a2}_k={k}.pdf')
        plt.show()


    def draw_error(a1,a2,k):
        predict_np = model.predict_U(x_test).cpu().detach().numpy()
        ...
        plt.tight_layout()
        plt.savefig(f'Helmholtz_error-RAM-AW_a1={a1}_a2={a2}_k={k}.pdf')
        plt.show()
    ...
    exact_u = lambda x: np.sin(a1 * np.pi * x[:, [0]]) * np.sin(a2 * np.pi * x[:, [1]])*(x[:, [0]]**2-1)*(x[:, [1]]**2-1)
