...
def GPT_residual(alpha, beta, gamma, c, xcos_x2cos2, out, 
                 Ptt_aPxx_bP_term):
    u = torch.matmul(out,  c)        
    f1 = torch.matmul(Ptt_aPxx_bP_term, c)
    f2 = torch.mul(gamma, torch.square(u))
    f3 = torch.add(f1, f2)
    return torch.add(f3, xcos_x2cos2)

class GPT(nn.Module):
    def __init__(self, alpha, beta, gamma, out, out_IC, out_IC_t, 
                 out_BC, xcos, fhat, Ptt_aPxx_bP_term, IC_u1, IC_u2, BC_u,L_hat=None):
        super().__init__()
        self.alpha = alpha
        self.beta  = beta
        self.gamma = gamma
    ...
    def loss(self, c):
        ...
        if self.weight is None:
            ...
            loss = loss_R + loss_IC1 + loss_IC2 + loss_BC
        else:
            #print(self.weight*c,sum(self.weight*c))
            loss = loss_R + self.loss_function(sum(self.weight*c),torch.ones(1).to(device))
        return loss  
