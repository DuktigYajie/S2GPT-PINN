...
class grad_descent(): 
    def __init__(self, alpha, beta, gamma, xt_size, IC_size, BC_size, IC_u1, 
                 ...
                 out_IC_t, out_BC, epochs_gpt, lr_gpt,L_hat=None):
        # PDE parameters
        self.alpha = alpha
        self.beta  = beta
        self.gamma = gamma
    ...
    def update(self, c, c_reshaped):
        ...
        if self.weight is None:
            ...
            grad_list += torch.mul(self.fac3, torch.sum(torch.mul(IC1_term, self.out_IC), axis=0))
            #######################################################################
            ########################  Initial 2 Gradient  #########################         
            IC2_term = torch.matmul(self.out_IC_t, c_reshaped)
            
            grad_list += torch.mul(self.fac3, torch.sum(torch.mul(IC2_term, self.out_IC_t), axis=0))
        else:
            IC_term = 2*(sum(self.weight*c_reshaped)-1)
            #print(IC_term)
            grad_list += torch.sum(torch.mul(IC_term,  self.weight), axis=0)
            #print(grad_list)
