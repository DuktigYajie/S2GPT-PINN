...
class GPT(nn.Module):
    def __init__(self, lmbda, eps, out, out_IC, out_BC_ub, out_BC_lb, 
                 out_BC_ub_x, out_BC_lb_x, fhat, Pt_lPxx_eP_term, IC_u,L_hat=None):
        super().__init__()        
        self.lmbda = lmbda
        self.eps   = eps
    ...
    def loss(self, c):
        ...
        if self.weight is None:
            ...
            loss = loss_R + loss_IC + loss_BC + loss_BC_x
        else:
            #print((self.weight*c).shape)
            loss_IBC = self.loss_function(sum(self.weight*c),torch.ones(1).to(device))
            loss = loss_R+10*loss_IBC
        return loss
